...
export const GET = apiRoute<{ id: string }>(async ({ request, params }) => {
  return ok(serializePost(await loadPost(params.id), request.nextUrl.origin));
});
...
export const PATCH = apiRoute<{ id: string }>(async ({ request, params, auth }) => {
  ...
  await updatePost(post.id, body.title ?? post.title, body.content ?? post.content, post.imagePath);

  revalidateTag("posts", { expire: 0 });
  revalidateTag(`post-${post.id}`, { expire: 0 });

  return ok(serializePost((await findPost(post.id))!, request.nextUrl.origin));
});
...
export const DELETE = apiRoute<{ id: string }>(async ({ params, auth }) => {
  ...
  await deletePost(post.id); // 댓글은 ON DELETE CASCADE 로 함께 삭제된다
