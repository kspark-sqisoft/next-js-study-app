// refresh_tokens 테이블 접근 함수. 공개 API 의 리프레시 토큰을 다룬다.
//
...
function findRow(token: string): RefreshTokenRow | null {
  const row = db
...
function insertToken(userId: number, familyId: string, expiresAt: Date): IssuedRefreshToken {
  ...
  db.prepare("INSERT INTO refresh_tokens (user_id, family_id, token_hash, expires_at) VALUES (?, ?, ?, ?)").run(
    userId,
    familyId,
...
/** 여러 문장을 하나의 트랜잭션으로. 중간에 실패하면 전부 되돌린다 (node:sqlite 는 동기라서 이렇게 감싸면 된다). */
...
function revokeFamily(familyId: string): number {
  const result = db
...
function pruneExpired(): void {
  const result = db.prepare("DELETE FROM refresh_tokens WHERE expires_at < ?").run(toSqlDate(new Date()));
  if (Number(result.changes) > 0) log.api(`  ↳ 만료된 리프레시 토큰 ${result.changes}건 정리`);
}
 ...
 */
export function issueRefreshToken(userId: number): IssuedRefreshToken {
  pruneExpired();
  const expiresAt = new Date(Date.now() + REFRESH_TOKEN_TTL_SECONDS * 1000);
  const issued = insertToken(userId, randomUUID(), expiresAt);
...
export function rotateRefreshToken(token: string): RotateResult {
  return transaction(() => {
    ...
    if (row.used_at !== null) {
      const revoked = revokeFamily(row.family_id);
      log.api(`  ↳ 리프레시 토큰 재사용 감지! 가족 ${short(row.family_id)} 의 토큰 ${revoked}건 폐기 (userId=${row.user_id})`);
      return { ok: false, reason: "reused" };
    }

    if (fromSqlDate(row.expires_at).getTime() <= Date.now()) return { ok: false, reason: "expired" };

    // 옛 토큰을 소비하고, 같은 가족·같은 만료 시각으로 새 토큰을 만든다
    db.prepare("UPDATE refresh_tokens SET used_at = datetime('now') WHERE id = ?").run(row.id);
    const next = insertToken(row.user_id, row.family_id, fromSqlDate(row.expires_at));
    log.api(`  ↳ 리프레시 토큰 회전: 가족 ${short(row.family_id)} 의 #${row.id} 소비 → 새 토큰 발급 (userId=${row.user_id})`);
 ...
 */
export function revokeRefreshTokenFamily(token: string): boolean {
  const row = findRow(token);
  if (!row) return false;
  const revoked = revokeFamily(row.family_id);
  log.api(`  ↳ 로그아웃: 가족 ${short(row.family_id)} 의 토큰 ${revoked}건 폐기 (userId=${row.user_id})`);
  return true;
}
