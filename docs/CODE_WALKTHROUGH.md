...
**서버 컴포넌트와 클라이언트 컴포넌트.** 파일 맨 위에 `"use client"` 가 없으면 서버 컴포넌트다. 서버에서만 실행되므로 DB 를 직접 읽을 수 있고, 그 코드는 브라우저로 전송되지 않는다. `"use client"` 가 있으면 서버에서 한 번 HTML 로 그려진 뒤 브라우저에서도 실행된다. `onClick`, `useState`, `useEffect` 는 클라이언트 컴포넌트에서만 쓸 수 있다.
