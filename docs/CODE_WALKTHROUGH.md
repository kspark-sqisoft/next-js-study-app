...
**서버 컴포넌트와 클라이언트 컴포넌트.** 파일 맨 위에 `"use client"` 가 없으면 서버 컴포넌트다. tRPC 에서는 이 구분이 더 중요하다. 서버 컴포넌트는 `src/trpc/server.tsx` 의 `trpc` 를, 클라이언트 컴포넌트는 `src/trpc/client.tsx` 의 `useTRPC()` 를 쓴다. 모양은 같고(`trpc.comments.list.queryOptions(...)`) 실행 경로가 다르다.
