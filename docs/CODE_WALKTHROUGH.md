...
5. **브라우저에서 hydration.** `StoreHydrator` 의 `useEffect` 가 zustand persist 스토어를 localStorage 에서 복원한다. 첫 방문부터 Link 이동, prefetch, 스트리밍까지 로딩의 전체 여정은 **부록 C** 에, 이 책의 용어는 **부록 D** 에 모아 두었다.
...
| 용어 | 뜻 | 어디 |
| --- | --- | --- |
| 데이터 접근 층 (DAL) | `src/lib/` 의 함수들. Prisma 쿼리는 여기에만, 세션 확인도 여기(`dal.ts`) | 0장, 6장 |
...
| 용어 | 뜻 | 어디 |
| --- | --- | --- |
| ORM | 테이블을 객체·메서드로 다루게 해 주는 층. SQL 을 대신 만든다 | 1장, 12장 |
| `schema.prisma` / 모델 | 테이블 정의의 단일 출처 / 테이블 하나에 대응하는 선언 (`model Post`) | 12장 |
| `@map` / `@@map` | 필드·모델 이름과 실제 컬럼·테이블 이름을 다르게 연결 (camelCase ↔ snake_case) | 12장 |
| `@relation` / `onDelete: Cascade` | 외래키 관계 선언 / 부모 삭제 시 자식도 삭제 | 7장 |
| 마이그레이션 | 스키마 변경을 SQL 파일로 남기고 순서대로 적용하는 것 (`prisma/migrations/`) | 1장, 11장 |
| `prisma generate` | 스키마에서 클라이언트 코드와 타입을 만드는 것 (`src/generated/prisma/`) | 1장 |
| 어댑터 | Prisma 7 이 실제 DB 드라이버를 받는 방식 (`@prisma/adapter-better-sqlite3`) | 1장 |
| `findUnique` / `findMany` / `count` | 유니크 키로 한 건 / 조건으로 여러 건 / 개수 | 4장, 12장 |
| `include` / `select` | 관계를 따라 함께 가져오기 (JOIN) / 필요한 컬럼만 고르기 | 2장, 4장 |
| `where` 객체 (`OR`, `AND`, `contains`, `lt`, `in`, `not`) | SQL 조건을 객체로 표현 | 4장, 12장 |
| `take` / `skip` | LIMIT / OFFSET | 4장 |
| `create` / `update` / `updateMany` / `deleteMany` | INSERT(만든 행 반환) / UPDATE / 여러 건 → `{ count }` / 없어도 조용히 | 3장 |
| `Prisma.PostGetPayload<{ include }>` | 특정 include 로 조회했을 때의 행 타입을 자동 생성 | 4장, 7장 |
| `$transaction(async (tx) => …)` | 콜백 안 쿼리를 하나의 트랜잭션으로. 반드시 `tx` 로 보내야 함 | 10장 |
| `sqlNow()` | `datetime('now')` 와 같은 형식의 문자열. TEXT 날짜 컬럼 호환용 | 8장, 12장 |
| `prisma:query` 로그 | 개발 모드에서 Prisma 가 실제로 보낸 SQL | 0장 |
...
| **확인하는 법** | `[session] getCurrentUser()` 로그가 한 요청에 한 번만 | `[cache] MISS` 유무, `cachedAt` 시각, `prisma:query` 줄의 유무 | `npm run build` 표의 `○`/`◐`, `.next/server/app/` 의 `.html`·`.rsc` 파일 | 프로덕션에서 Network 탭의 `_rsc` 요청이 있는지 없는지 |
